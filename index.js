...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    password[i] = characters.at(index);
  }
...
function calculateVolumeCylinder(radius, height) {
  ...
  let res = Math.PI * Math.pow(radius, 2) * height;
...
function getFractionalPart(num) {
  ...
  let numRound = Math.trunc(num);
  let diff = num - numRound;
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let res = Math.round(principal * Math.pow(1 + interestRate, years));
  return res;
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let totalPrice = products.reduce((total, price) => total + price.price, 0);
